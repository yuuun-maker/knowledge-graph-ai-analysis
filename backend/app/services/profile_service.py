...
class ProfileService:
    ...
    def avatar_url_for(user_id: int, stored: str = None) -> str:
        ...
        {user_id}_{时间戳}{扩展名} 独立存在磁盘上。项目里 app.db 是 git 跟踪的
        二进制文件，历史上发生过被合并脚本整体覆盖、指针丢失而图片还在的事故
        （见 backup/dbmerge_20260917/），此时扫目录仍能取到最新头像，
        用户不必重新上传。注意 _find_avatar 取的是字典序最后一个，
