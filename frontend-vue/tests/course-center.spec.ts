...
import {
  BASE, RUN, teacherName, studentName, registerAndLogin, api, dismissAlert, clickRadio,
} from './helpers'

/**
 * 课程中心全流程（对应验收标准）
 * 教师：登录 → 创建课程 → 生成加入码 → 邀请学生 → 审核申请 → 查看学生
 * 学生：注册 → 加课码加入 → 等待/通过 → 我的课程 → 进入课程工作区
 *
...
test('教师：创建课程（审核制 / 直接加入）→ 卡片显示 8 位加课码', async ({ page }) => {
  ...
  // 侧边栏有「课程中心」，且默认落在「我的课程」
  await expect(page.getByRole('tab', { name: '我的课程' })).toBeVisible()

  // ---- 课程一：审核后加入 ----
  await page.getByRole('button', { name: '创建课程' }).first().click()
  ...
  await expect(card1.getByText('成员')).toBeVisible()
...
test('教师：学生管理 / 邀请弹窗 / 课程设置 可用', async ({ page }) => {
  ...
  await expect(page.getByRole('tab', { name: /待审核申请/ })).toBeVisible({ timeout: 15000 })
  ...
  await expect(page.getByText('新生成的链接')).toBeVisible({ timeout: 15000 })
  await expect(page.locator('.invite-link-row input')).toHaveValue(/\/invite\/.+/)
  await page.getByRole('dialog').getByRole('button', { name: '关闭', exact: true }).click()

  // 课程设置（加课码 + 刷新 + 删除入口）
  await page.getByRole('button', { name: '返回课程列表' }).click()
  await page.locator('.course-card', { hasText: COURSE_APPROVAL })
    .getByRole('button', { name: '课程设置' }).click()
...
test('教师：审核申请 → 学生成为正式成员', async ({ page }) => {
  ...
  // 切到「全部成员」应能看到该学生
  await page.getByRole('tab', { name: /全部成员/ }).click()
